...
def convert_from_npz(input,outtype):
...
def convert_to_npz(input,outtype):

    pcd = o3d.io.read_point_cloud(input)
    np.savez(os.path.splitext(input)[0]+"."+outtype, points=np.asarray(pcd.points), colors=np.asarray(pcd.colors), normals=np.asarray(pcd.normals))

    print("Pointcloud saved to ",os.path.splitext(input)[0]+"."+outtype)


def convert_pc(input,outtype):

    if os.path.splitext(input)[1] == ".npz":
        convert_from_npz(input,outtype)
        return
    if outtype == "npz":
        convert_to_npz(input,outtype)
        return
...
def convert_mesh(input,outtype):
...
if __name__ == '__main__':
    ...
    if args.mode == "pc":
        convert_pc(args.input,args.type)
    elif args.mode == "mesh":
        convert_mesh(args.input,args.type)
    else:
