...
for i,m in enumerate(methods):
    ...
    ax.bar(space_between_experiments * i, top, width=0.9, color=colors[i])

    if experiments[ek] == "components":
        ax.text((space_between_experiments * i) - 0.27,
                top + 0.3, "{:.1f}".format(top), color='black', fontweight='bold')
    else:
        ax.text((space_between_experiments * i) - 0.37,
                top * 1.07, "{:0.2g}".format(top), color='black', fontweight='bold')
...
if experiments[ek] == "runtime":
    ax.set_yscale('log')
elif experiments[ek] == "components":
